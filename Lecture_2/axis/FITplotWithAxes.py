...
Updated on Thu OCT 19 2017, 23.05.2018 pyfits -> astropy.io
Lecture 2.8/9  Improvement of axis labels, different color-tables and filtering
...
plt.xlabel('Relative time [s]',fontsize=15)
plt.ylabel('Frequency [MHz]',fontsize=15)
plt.title('FIT file presentation as Gaussian smoothed data, zoomed in time',fontsize=15)
...
hh = float(fits.open(files)[0].header['TIME-OBS'].split(":")[0])
mm = float(fits.open(files)[0].header['TIME-OBS'].split(":")[1])
ss = float(fits.open(files)[0].header['TIME-OBS'].split(":")[2])
...
dt = fits.open(files)[0].header['CDELT1'] # read time resolution from FIT-file
print (dt)
...
for j in range(len(a)-1):
    ...
    tst = '{:02.0f}:{:02.0f}:{:02.0f}'.format(hh,mm,ss)
    ahms.append(tst)
...
mplot.title("File: "+files, fontsize=20)
